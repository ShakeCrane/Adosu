...
**续接是交付合同的一部分。** 本次已确认的技术疏漏是完成自身任务后未及时续接下一阶段。今后在合同满足后，除写回压缩报告外，必须立即按已知依赖推进后续交接：在权限允许时将自己的子 Issue 推进至 `done`，或明确通知可被唤醒的验收责任人；不得只提交报告就结束 Run，让父 Stage 停在无人续接状态。若受权限、工作目录、平台唤醒或依赖阻塞，必须明确报告 `BLOCKED`、阻塞原因、责任人和恢复条件。
