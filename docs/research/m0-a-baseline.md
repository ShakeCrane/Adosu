...
- The executable micro/regression tests originally passed as eighteen, then twenty-five after the P1 fixes, thirty-one after the P2 duration fix and thirty-four after the Pause canonical-tempo follow-up. Four follow-up regressions are now registered and pass in the local Release .NET run; no CI result is claimed. The suite includes both real supplied fixtures: ordering across floors, malformed-floor retention, non-object action retention, source-order preservation, duration-legality rejection, and the real-fixture statistics.
...
- `Pause` raw duration is checked in source order and committed after canonical floor-end tempo resolution. Accumulated beats, derived seconds and floor end must be finite and non-negative; only the offending candidate is rejected. A legal `Pause` retains its existing `ADF-PAUSE-INFERRED` warning-level semantic and its provenance, so Pause is still not promoted past `INFERRED`.
- A final canonical time invariant checks finite floor starts and event/segment times. Individually finite spans may still overflow the cumulative timeline; the follow-up fails closed with a floor-identified `ADF-DURATION-VALUE` `FormatException` before returning such a chart.
...
- `ADF-PAUSE-INFERRED` fires only when an accepted Pause contributes positive beats, and points to the first such accepted Pause; it remains `INFERRED`, never `VERIFIED`.

### Follow-up invariants (2026-09-21)

- Source-application ordinals are reserved in each floor's original source order before deferred Pause/SetSpeed processing; directly rejected and deferred-rejected Pauses both retain real ordinals. SetSpeed nonzero angle offsets still determine their own timing boundaries.
- A cumulative floor endpoint outside finite double range fails closed with floor-identified `ADF-DURATION-VALUE` `FormatException` rather than returning a corrupted canonical chart or fabricating a gap.
- 38 cases registered: 34 preceding cases and four focused regressions. The local .NET build and ignored-fixture run pass for this revision; no CI result is claimed.
